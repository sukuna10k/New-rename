...
async def rename(bot, update):
    ...
    await update.message.delete()
    await update.message.reply_text(f"__Veuillez entrer un nouveau nom...__\n\n**Note :** Extension Non requis", reply_to_message_id=id,
...
async def doc(bot, update):
    ...
    hinata = message
    ms = await update.message.edit("🚀 Téléchargement...  ⚡")
    used_limit(update.from_user.id, file.file_size)
    c_time = time.time()
    total_used = used + int(file.file_size)
    used_limit(update.from_user.id, total_used)
    try:
        path = await bot.download_media(message=file, progress=progress_for_pyrogram, progress_args=("🚀 Téléchargement...  ⚡",  ms, c_time))
    ...
    if value < file.file_size:
        await ms.edit("🚀 Téléchargement...  ⚡")
        try:
            filw = await app.send_document(LOG_CHANNEL, document=metadata_path if _bool_metadata else file_path, thumb=ph_path, caption=caption, progress=progress_for_pyrogram, progress_args=("🚀 Téléversement...  ⚡",  ms, c_time))
        ...
        await ms.edit("🚀 Téléversement...  ⚡")
        c_time = time.time()
        try:
            await bot.send_document(update.from_user.id, document=metadata_path if _bool_metadata else file_path, thumb=ph_path, caption=caption, progress=progress_for_pyrogram, progress_args=("🚀 Téléversement...  ⚡",  ms, c_time))
            await ms.delete()
...
async def vid(bot, update):
    ...
    if value < file.file_size:
        await ms.edit("🚀 Téléversement...  ⚡")
        try:
            filw = await app.send_video(LOG_CHANNEL, video=metadata_path if _bool_metadata else file_path, thumb=ph_path, duration=duration, caption=caption, progress=progress_for_pyrogram, progress_args=("🚀 Téléversement.....  ⚡",  ms, c_time))
            from_chat = filw.chat.id
            mg_id = filw.id
            time.sleep(2)
            await bot.copy_message(update.from_user.id, from_chat, mg_id)
